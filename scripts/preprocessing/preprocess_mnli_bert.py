...
def preprocess_MNLI_data(inputdir,
    ...
    print("\t* Reading data...")
    data = preprocessor.read_data_bert(os.path.join(inputdir, train_file))
    ...
    data = preprocessor.read_data_bert(os.path.join(inputdir, matched_dev_file))
    with open(os.path.join(targetdir, "bert_matched_dev_data.pkl"), "wb") as pkl_file:
        pickle.dump(data, pkl_file)

    print("\t* Reading mismatched dev data...")
    data = preprocessor.read_data_bert(os.path.join(inputdir, mismatched_dev_file))
    with open(os.path.join(targetdir, "bert_mismatched_dev_data.pkl"), "wb") as pkl_file:
        pickle.dump(data, pkl_file)

    # -------------------- Test data preprocessing -------------------- #
    print(20*"=", " Preprocessing test sets ", 20*"=")
    print("\t* Reading matched test data...")
    data = preprocessor.read_data_bert(os.path.join(inputdir, matched_test_file))
    print("\t* Saving result...")
    with open(os.path.join(targetdir, "bert_matched_test_data.pkl"), "wb") as pkl_file:
        pickle.dump(data, pkl_file)

    print("\t* Reading mismatched test data...")
    data = preprocessor.read_data_bert(os.path.join(inputdir, mismatched_test_file))
    print("\t* Saving result...")
    with open(os.path.join(targetdir, "bert_mismatched_test_data.pkl"), "wb") as pkl_file:
        pickle.dump(data, pkl_file)
...
if __name__ == "__main__":
    ...
    preprocess_MNLI_data(
        ...
        labeldict=dict(), #config["labeldict"]
        bos=config["bos"],
        eos=config["eos"]
    )
