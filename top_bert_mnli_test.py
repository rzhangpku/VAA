...
def transform_batch_data(data, batch_size=64, shuffle=True):
    ...
    for i in range(len(index)):
        ...
        data_batch['id'][idx].append(data['ids'][index[i]])
    return data_batch
...
def main(train_file,
    ...
    # Compute loss and accuracy before starting (or resuming) training.
    data = test(model, valid_dataloader, criterion)
    data.to_csv('matched_submission.csv', index=False)

    data = test(model, test_dataloader, criterion)
    data.to_csv('mismatched_submission.csv', index=False)


if __name__ == "__main__":
    default_config = "../../config/testing/mnli_testing_bert.json"
    ...
    parser.add_argument("--checkpoint_model0",
                        default=os.path.dirname(os.path.realpath(__file__)) + '/data/checkpoints/MNLI/bert/' +"best.pth.tar",
                        help="Path to a checkpoint file to resume training")

    parser.add_argument("--checkpoint_model1",
                        default=os.path.dirname(os.path.realpath(__file__)) + '/data/checkpoints/MNLI/bert/' +"esim_model1{}.pth.tar".format(12),
                        help="Path to a checkpoint file to resume training")
